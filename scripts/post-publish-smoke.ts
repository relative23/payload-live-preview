 ...
 * component needs the Astro compiler and `middleware-entry` imports the
 * integration's virtual module. An unlisted subpath fails the smoke test.
 * This is the 1.8.1 export map; main's table lists the 2.x one.
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // The optional peer is installed so the entries that need it are genuinely
    // exercised rather than skipped: codegen needs `ts-morph`.
    run(
      'npm',
      [
        'install',
