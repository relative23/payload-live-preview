/**
 * How the byte gates measure: the exact raw, gzip and Brotli sizes, and every
 * exceeded dimension of a budget rather than the first one. The numbers live
 * in `bundle-budgets.ts` (the inline profiles) and `entry-budgets.ts` (the
 * package entries), each with its own log.
 */
