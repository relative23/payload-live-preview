...
The tag goes into `<head>`. The script stays inert outside the admin's preview iframe and costs about 36 KB gzip on every page that carries it. `generateInlineScript()` returns the script body and accepts the runtime options ([options.md](options.md)); `wrapWithScriptTag()` adds the tag and an optional `nonce`.
