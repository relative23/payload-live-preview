...
| `LP0807` | A revision changed a field with no binding; `onUnboundChange: 'route'` refreshed the route              | Informational: the field has no `data-payload-field` anywhere on the page, so the route was refreshed instead of patched. Bind the field to patch it in place, or set `onUnboundChange: 'ignore'` to accept the stale value.                                                                                                                                                                                         |
