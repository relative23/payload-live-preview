/**
 * `definePreview()`: origin, route and depth bound once, so the initial draft
 * read and the runtime's `mergeDepth` cannot drift apart. Every read takes an
 * explicit authorization verdict; only a real context reads the draft.
 */
...
/** What every read takes from the bound configuration. */
interface ReadSettings {
  readonly config: PreviewServerConfig;
  readonly timeoutMs: number;
}

/** One read in flight: what the diagnostic and a thrown failure need to say. */
...
function params(depth: number, options: PreviewReadOptions, draft: boolean): URLSearchParams {
...
/** One GET against the REST API; every exit reports to `onDiagnostic` and is shaped as a result. */
async function read<T>(
  { config, timeoutMs }: ReadSettings,
  url: string,
...
/** Report the outcome, then hand it back — or throw it, in `errorMode: 'throw'`. */
function settle<T>(attempt: ReadAttempt, result: PreviewFetchResult<T>): PreviewFetchResult<T> {
  const { config, url, draft } = attempt;
