...
Requirements: Node.js >= 22.22 and the npm version declared by
`packageManager`. npm 12 needs Node 22.22 or newer, so development does too;
the published package still supports Node >= 20.19 (`engines`).
...
| `mutation-policy.ts` · `npm run test:mutation:policy[:pr]`                                 | The reviewed Stryker outcome: exact scope, total, no-coverage, error and ignored counts; the score within `mutationScoreDriftMutants`; timeouts under a ceiling                                                    | `quality/mutation-policy.json`, `quality/mutation-policy-pr.json`          | Record the numbers of a fresh run by hand; the procedure is in [docs/testing.md](docs/testing.md). An improvement outside the drift band fails too until recorded; fewer timeouts only print a notice |
...
| `check-bundle-size.ts` · `npm run test:bundle` (inside `npm run build`)                    | Raw, gzip and Brotli budgets per entry and for the inline script                                                                                                                                                   | `scripts/bundle-budgets.ts`, `ENTRY_BUDGETS` in `scripts/entry-budgets.ts` | Raise a budget in the commit that spends it, with the reason in the ledger comment; a drop above 2 % prints a notice to tighten it                                                                    |
| `check-tree-shaking.ts` · `npm run test:treeshake` (inside `npm run build`)                | Gzip budgets for one-symbol consumers bundled with Vite through `node_modules`, so `exports` and `sideEffects` decide what ships                                                                                   | `TREE_SHAKING_FIXTURES` in the script                                      | Raise a budget in the commit that spends it, with the reason in the comment above the table; a drop above 2 % prints a notice to tighten it                                                           |
...
| `release-gate.ts` · Release workflow                                                       | Only a completed, successful CI push run on `main` or `release/1.x` of this repository is published; only `main` gets a Version PR                                                                                 | None; the verdict is computed from Git, npm and GitHub state               | `workflow_dispatch` with the CI `run_id` re-enters a certified run; a tag on another commit is an error, not repaired                                                                                 |
...
| `check-generated-runtime.ts` · `npm run check:generated`                                   | The committed `src/inline/runtime-lean.generated.ts` is what the source builds, down to the package version a Version PR changes                                                                                   | None; `npm run build:runtime` makes it again and git compares the two      | `npm run build:runtime`, then commit the file with the change that altered it                                                                                                                         |
| `check-trusted-core.ts` · `npm run test:architecture`                                      | The trusted core's line count and its imports from outside itself, at the measured numbers                                                                                                                         | `quality/trusted-core.json`                                                | Raise the line limit or add the import with the reason in the same file; `docs/audit.md` explains what the core is                                                                                    |
| `check-complexity.ts` · `npm run test:architecture`                                        | The size of the public surface: exported names per API report, entry subpaths, adapter and inline options, diagnostic codes, source modules; a report without a budget or a budget without a report fails          | `quality/complexity-budget.json`                                           | Raise the limit and write why next to it; a total more than 2 % under its limit prints a notice to lower it                                                                                           |
| `compare-scope.ts --check` · `npm run test:architecture`                                   | The code-line count of the scope compared with `@payloadcms/live-preview`, the number the comparison quotes                                                                                                        | `REVIEWED_SCOPE_LINES` in the script                                       | Move the constant with the count, in either direction, and say why in the commit                                                                                                                      |
| `architecture-rules.ts` + `sink-rules.ts` · `npm run test:architecture`                    | Which modules may hold a capability (message listener, fetch, credentials, Trusted Types policy, HTML sink), and each sink site's justification                                                                    | `quality/trusted-core.json` (modules), `scripts/sink-inventory.ts` (sites) | List the module or site with the justification that applies; three capabilities cannot be held outside the core at all                                                                                |
| `mutation-policy.ts` · `npm run test:mutation:policy:core`                                 | The Stryker outcome on the trusted core alone, on every push to `main` and by hand with `STRYKER_SCOPE=core`: exact, with no score drift band, and timeouts under a ceiling                                        | `quality/mutation-policy-core.json`                                        | Record the numbers of a fresh run; a survivor is a missing test or a line that does not belong in the core                                                                                            |
| `check-interaction-budgets.ts` · `npm run test:interaction` (inside `npm run build`)       | Merge requests per 18-keystroke burst and the p95 from a keystroke to the page, per scenario, in jsdom against an echoing merge endpoint                                                                           | `scripts/interaction-budgets.ts`                                           | Move the number in the commit that moves it, in either direction: an improvement nobody records fails as loudly as a regression                                                                       |
| `check-protocol-semantics.ts` · `npm run test:protocol-semantics` (inside `npm run build`) | What each recorded wire corpus _means_ when replayed through the runtime — a session that crossed a save, what a relationship event names — not what it looks like                                                 | `tests/fixtures/protocol-model.ts`                                         | Add or change the row for the field, with its holder; a known defect is an exception in the model that names the task removing it                                                                     |
| `check-upstream-findings.ts` · `npm run test:upstream-findings`                            | The behaviours `docs/react.md` measures the official hook against still reproduce on the _published_ `@payloadcms/live-preview`; needs the registry, so it runs in `protocol-watch.yml`, not in `check` or `build` | `docs/react.md` and `tests/unit/adapters/payload-hook-comparison.test.ts`  | When the published package changes behaviour, change the comparison and the page in the same commit, naming the version that changed it                                                               |
...
2. A successful CI push run triggers the Release workflow (`workflow_run`).
   ...
   changesets → Version PR; it is, with no changesets and no GitHub Release yet
   → publish, which skips the upload when npm holds the certified bytes and
   creates the missing tag and Release; the GitHub Release exists → nothing.
